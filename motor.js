/* ============================================================================
   ...
   Alles, was in jedem Level gleich ist: die eigene Schrift, der Bild-Cache,
   die Zeichenbefehle, die Tonbausteine und der Aufbau der Leinwand.

   Das lag bis jetzt Wort fuer Wort in jeder Leveldatei - rund 180 Zeilen
   doppelt. Ein Fehler im Font haette in zwei Dateien behoben werden
   muessen, und beim dritten Level in drei. Ein neues Level laedt jetzt
   motor.js und faengt bei seinen eigenen Sachen an.

   Was hier NICHT hingehoert und in den Leveln bleibt: die Palette P, die
   Sprite-Farben KEY, die Sprites selbst, die Regler in TUNE und alles,
   was mit dem Level zu tun hat.

   Bewusst ein normales Script wie fortschritt.js: ein Doppelklick auf
   index.html soll reichen, Module scheitern ueber file:// an CORS.

   Reihenfolge in der Seite:
     <script src="fortschritt.js"></script>   Spielstand und Levelliste
     <script src="motor.js"></script>         dieser Motor
     <script> ... das Level ... </script>
   ========================================================================== */

/* ==========================================================================
   3x5 BITMAP-FONT
   Eine eigene Schrift statt der Browser-Schrift: bei 320x180 ist jede
   echte Schriftart entweder unscharf oder zu gross.
   ========================================================================== */
const F = {
...
/* ==========================================================================
   ...
   Sprites, Konturen, Schrift und Farbverlaeufe werden einmal auf eine
   eigene Leinwand gemalt und danach nur noch kopiert. Das war der
   Unterschied zwischen 4,8 und 1,2 ms Zeichenzeit pro Bild.
   ========================================================================== */
...
/* Welcher Buchstabe im Sprite welche Farbe ist. Setzt jedes Level selbst,
   weil die Raeume unterschiedlich aussehen. */
let KEY={};
...
/* Dunkler Rand um eine Figur, damit sie sich von der Wand abhebt.
   Der Vorgabewert ist nur Formsache - jede Stelle im Spiel gibt ihre
   Deckkraft selbst an. */
...
/* ==========================================================================
   TON
   Nur die Bausteine. Was daraus wird - Schritte, Alarm, Musik - steht im
   Level, weil jedes Level anders klingt.
   ========================================================================== */
let AC=null, muted=false;
/* Wird einmal aufgerufen, sobald es Ton gibt. Die Level haengen hier den
   Start ihrer Musik ein: ohne das laeuft der Taktzeiger von 0 los und
   schiebt beim ersten Ton tausende Noten aus der Vergangenheit nach. */
let beiAudioStart=null;
function ensureAudio(){ if(AC) return AC;
  try{ AC=new (window.AudioContext||window.webkitAudioContext)();
       if(beiAudioStart) beiAudioStart(AC); }
...
if(_fs) _fs.addEventListener('click',e=>{e.preventDefault();vollbild();});
